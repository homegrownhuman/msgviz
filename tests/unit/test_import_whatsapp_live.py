...
def test_full_ingest(home) -> None:
    ...
    assert stats["new"] == 8           # 4 one-to-one + 4 group
    assert stats["skipped_existing"] == 0
    con = _db(home)
    assert con.execute("SELECT COUNT(*) FROM message").fetchone()[0] == 8
...
def test_reimport_is_incremental(home) -> None:
    ...
    stats2 = _import()
    assert stats2["new"] == 0
    assert stats2["skipped_existing"] == 8
    con = _db(home)
    # No duplication.
    assert con.execute("SELECT COUNT(*) FROM message").fetchone()[0] == 8
    con.close()
...
def test_dry_run_writes_nothing(home) -> None:
    ...
    assert stats["new"] == 8           # would-import count
    con = _db(home)
